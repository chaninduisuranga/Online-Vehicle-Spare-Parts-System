...
public class UserDB {
	...
	public static User isValidUser(String u_email, String u_password) {
		try {
			...
			String query = "SELECT * FROM user WHERE u_email = '" + u_email + "' AND u_password = '" + u_password
					+ "' LIMIT 1";
	...
	public static boolean registerUser(String u_firstname, String u_lastname, String u_email, String u_password,
			String u_type, String u_image) {
		try {
			Connection con = DBConnection.getConnection();
			Statement stmt = con.createStatement();
			String query = "INSERT INTO User (u_firstname, u_lastname, u_email, u_password, u_type, u_image) VALUES ('"
					+ u_firstname + "', '" + u_lastname + "', '" + u_email + "', '" + u_password + "', '" + u_type
					+ "', '" + u_image + "')";
	...
	public static boolean updateUser(int u_id, String u_firstname, String u_lastname, String u_email) {
		...
		try {
			Connection con = DBConnection.getConnection();
			Statement stmt = con.createStatement();
			String query = "UPDATE user SET u_firstname = '" + u_firstname + "', u_lastname = '" + u_lastname
					+ "', u_email = '" + u_email + "' WHERE u_id = '" + u_id + "'";
